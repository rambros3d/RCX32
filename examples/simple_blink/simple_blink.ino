...
void setup() {
  ...
  lights.addLed(LED2, ledPin2);  // Multiple leds can be added to one type
  lights.addLed(LED2, ledPin3);  // Controlling LED2 will contol both ledPin2 & ledPin3
}

void loop() {
  // blink the LEDs
  // blink the headlight led for 800ms, then off for 200ms
  lights.blink(LED1, 800, 200);
  // blink the taillight led for 500ms, then off for 500ms
  lights.blink(LED2, 500, 500);
